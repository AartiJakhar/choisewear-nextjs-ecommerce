...
export default function Checkout({ addToCart, cart, removeFromCart, clearCart, subtotal }: any) {
  ...
  const initiatePayment = async () => {
    ...
    let dataa = { cart, subtotal, oid, email: "emailkkkkkkkkk" };
  ...
  return (
    <div className='container m-auto'>
      ...
      :<button className='flex mt-16 text-white bg-pink-500 border-0 py-2 px-4 focus:outline-none hover:bg-pink-500 rounded text-sm' disabled={Object.keys(cart).length === 0} >Case On Delivery ₹ {subtotal} </button> }
        </div>
      </div>
    </div>
  )
